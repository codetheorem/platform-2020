    ...
    <div class="login-container">
      <content-container v-if="!loginButtonClicked">
        ...
        <template v-slot:body>
          <p class="description-text">Welcome to Hack the Mountains! To register, please enter your email so we can send you a magic link to access the virtual platform for our event.</p>
          ...
          <form @submit.prevent="sendMagicLink">
            ...
            <div class="login-footer">
              ...
              <router-link to="login">
                <Button size="lg" text="Log In" :outlineStyle="true"/>
              </router-link>
            </div>
          </form>
        </template>
      ...
      <content-container v-else>
        ...
        <template v-slot:body>
          <p class="description-text" style="margin-bottom: 3rem">We sent an email to <b>{{ userEmail }}</b>! It has a magic link to help sign you in to the Hack the Mountains virtual platform.</p>
  ...
  methods: {
    async sendMagicLink() {
      if (this.userEmail !== '' && this.userEmail.includes('@')) {
        ...
        await this.performPostRequest(this.getEnvVariable('USERS_BASE_ENDPOINT'), 'add_user', params);
      } else {
