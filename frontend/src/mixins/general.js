...
export default {
  methods: {
    ...
    async setUserSlackId() {
      const user = await this.performGetRequest(this.getEnvVariable('USERS_BASE_ENDPOINT'), 'get_user', { id: this.getUserId() });
      if (!user.slack_id) {
        const slackId = await this.performGetRequest(this.getEnvVariable('PROJECTS_BASE_ENDPOINT'), 'lookup_user_slack_id_by_email', { email: user.email });
        if (slackId.user) {
          const rawSlackId = slackId.user.id;
          await this.performPostRequest(this.getEnvVariable('USERS_BASE_ENDPOINT'), 'update_user', { id: this.getUserId(), slack_id: rawSlackId });
        }
      }
    },
  },
